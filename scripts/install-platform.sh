...
# Usage function
usage() {
  echo -e "${CYAN}Invisible Platform Installer v${SCRIPT_VERSION}${NC}"
  ...
  echo "  sudo $0 --non-interactive -u user -p pass"
}
...
confirm_setup() {
  ...
  echo "Access Method: IP-based access (auto-detected)"
  echo ""
...
run_setup() {
  ...
  docker run --rm -it \
    ...
    scripts/deploy-to-k8s.sh "${DEPLOY_ARGS[@]}"
}
